...
const LessonItem = ({ lesson, onPress }) => (
  <TouchableOpacity style={styles.lessonCard} onPress={onPress}>
    <View style={styles.playIconBg}>
      <Icon name={lesson.type === 'folder' ? 'folder' : lesson.type === 'pdf' ? 'file-pdf-box' : 'play'} size={24} color="#6d28d9" />
    </View>
    <View style={{ flex: 1, marginLeft: 12 }}>
      <Text style={styles.lessonTitle}>{lesson.name}</Text>
      <View style={styles.progressBar}>
...
export default function CourseDetailsScreen({ route, navigation }) {
  ...
  const openItem = (item) => {
    ...
    // Prefer raw url; NO embedUrl anywhere
  ...
  return (
    <ScrollView style={styles.wrap}>
      <Image
        source={{ uri: course.bgImageUrl || 'https://img.freepik.com/free-vector/user-interface-design-concept-illustration_114360-1126.jpg' }}
        style={styles.headerImage}
      />
      ...
      <View style={styles.content}>
        ...
        {loading ? (
          ...
          lessons.map(lesson => (
            <LessonItem key={lesson.id} lesson={lesson} onPress={() => openItem(lesson)} />
          ))
        )}
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  lessonDuration: { fontSize: 14, color: '#6b7280', fontWeight: '600' },
  progressBar: {
    height: 6, backgroundColor: '#e5e7eb', borderRadius: 3, marginTop: 8, width: '80%',
  },
  progress: { height: 6, backgroundColor: '#8b5cf6', borderRadius: 3, width: '30%' },
});
